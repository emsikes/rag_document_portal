...
class SingleDocIngestor:
    def __init__(self, data_dir: str = "data/single_document_chat", faiss_dir: str = "faiss_index"):
        try:
            ...
            self.log.info("SingleDocumentIngestor initialized", temp_path=str(self.data_dir), faiss_dir=str(self.faiss_dir))
    ...
    def create_retriever(self, documents):
        try:
            ...
            self.log.info("Docoments split into chunks", count=len(chunks))

            embeddings = self.model_loader.get_embeddings()
            vectorstore = FAISS.from_documents(documents=chunks, embedding=embeddings)
            vectorstore.save_local(str(self.faiss_dir))
            self.log.error("FAISS index created and saved", faiss_path=str(self.faiss_dir))
